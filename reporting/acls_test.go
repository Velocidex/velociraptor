...
func (self *ACLTestSuite) TestNotebookPublicACL() {
	...
	assert.True(self.T(), reporting.CheckNotebookAccess(new_notebook, "User1"))
	...
	// User1 lost access.
	assert.False(self.T(), reporting.CheckNotebookAccess(new_notebook, "User1"))

	// The creator always has access regardless
	assert.True(self.T(), reporting.CheckNotebookAccess(new_notebook, "Creator"))
	...
	err = reporting.UpdateShareIndex(self.ConfigObj, new_notebook)
	assert.NoError(self.T(), err)
	...
	notebooks, err := reporting.GetSharedNotebooks(self.ConfigObj, "User1", 0, 100)
	assert.NoError(self.T(), err)
