...
ClientsListController.prototype.onClientsFetched = function(items) {
  angular.forEach(items, function(item) {
    ...
    item['_mac_addresses'] = [];
    angular.forEach(['interfaces'], function(iface) {
      if (angular.isDefined(iface['mac_address'])) {
        item['_mac_addresses'].push(iface['mac_address']);
      }
    }.bind(this));

    item['_usernames'] = [];
    angular.forEach(item['users'], function(user) {
      item['_usernames'].push(user['username']);
    }.bind(this));
