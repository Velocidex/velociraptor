...
class ClientFlowsView extends React.Component {
    ...
    componentDidUpdate = (prevProps, prevState, rootNode) => {
        let old_client_id = prevProps.client && prevProps.client
            .client_id;
    ...
    fetchFlows = () => {
        let client_id = this.props.client && this.props.client
        .client_id;
        if (!client_id) {
            return;
        }
        ...
        api.get("v1/GetClientFlows/" + client_id, {
            ...
            .then(response => {
                ...
                let selected_flow_id = this.state.currentFlow
                    .session_id;

                // If the router specifies a selected flow id, we select it.
                if (!this.state.init_router) {
                    selected_flow_id = this.props.match && this
                        .props.match.params &&
    ...
    setSelectedFlow = (flow) => {
        ...
        if (tab) {
            this.props.history.push(
                "/collected/" + this.props.client.client_id + "/" +
                flow.session_id + "/" + tab);
        } else {
            this.props.history.push(
                "/collected/" + this.props.client.client_id + "/" +
